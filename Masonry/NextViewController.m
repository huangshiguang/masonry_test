...
-(UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath{
    NextModel *model = self.arrModel[indexPath.row];
    NextTableViewCell *cell =[tableView dequeueReusableCellWithIdentifier:@"NextTableViewCell"];
    cell.homeModel = model;//数据填充
    model.cellHeight = [cell rowHeightWithCellModel];//model赋值
    return cell;
}
...
-(NSMutableArray *)arr
{
    if(_arr==nil){
        ...
        for(int i=0;i<_arr.count;i++){
            NextModel *m=[[NextModel alloc]init];
